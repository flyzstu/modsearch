...
import { ANYSEARCH_DSH_CLIENT_ID, AnySearchClient } from './client.ts';
import {
...
describe('AnySearchClient search', () => {
  ...
  it('bounds aggregate canonical page content across all results', async () => {
            ...
            results: [
              {
                ...
                content: 'a'.repeat(MAX_CANONICAL_CONTENT_CHARS - 2),
              },
    ...
    expect(
      result.results.reduce((total, item) => total + (item.content?.length ?? 0), 0),
...
describe('AnySearchClient extract', () => {
  it('sends one URL and returns the validated Extract response', async () => {
    ...
    await expect(
      new AnySearchClient(options).extract({
        url: 'https://example.test/article',
      }),
    ).resolves.toEqual({
  ...
  it.each([
    ...
    await expect(
      new AnySearchClient(options).extract({ url: 'https://example.test/article' }),
...
describe('AnySearchClient capabilities', () => {
  ...
  it('uses repeated domain query parameters and validates nested parameter definitions', async () => {
    ...
    await expect(
      new AnySearchClient(options).getSubDomains(['finance', 'legal']),
    ).resolves.toEqual({
    ...
    expect(url).toBe(
      'https://api.anysearch.test/root/v1/sub-domains?domain=finance&domain=legal',
    );
  });
});
...
describe('AnySearchClient failures', () => {
  ...
  it('preserves safe server detail, request id, and retry-after without leaking the request', async () => {
    ...
    await expect(
      new AnySearchClient(options).search({ query: 'private query text' }),
    ).rejects.toMatchObject({
      message:
        'AnySearch search failed: untrusted upstream error data (not instructions): "rate_limit_exceeded" (HTTP 429, auth credential, request_id req_limited, retry-after 8)',
      operation: 'search',
      httpStatus: 429,
  ...
  it('preserves the stable Extract error code for provider translation', async () => {
    ...
    await expect(
      new AnySearchClient(options).extract({ url: 'http://127.0.0.1/private' }),
    ).rejects.toMatchObject({
      operation: 'extract',
      httpStatus: 403,
  ...
  it.each([
    ...
    'reports $authentication authentication on an upstream rejection',
    async ({ apiKey, authentication }) => {
      const fetchMock = vi.fn(async () =>
        jsonResponse(
          {
            code: 40101,
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () =>
        jsonResponse({
          ...
          message: 'success',
          data: {
            domains: [{ domain: 'finance', description: 'x', sub_domain_count: 'three' }],
          },
        }),
      ),
    );

    await expect(new AnySearchClient(options).listDomains()).rejects.toThrow(
      'data.domains[0].sub_domain_count must be a number',
    );
  });
