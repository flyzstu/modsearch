...
import { ANYSEARCH_TOOL_TIMEOUT_MS, MAX_CAPABILITY_DOMAINS } from '../limits.ts';
import type { AnySearchDomainsResponse, AnySearchSubDomainsResponse } from '../types.ts';

export { MAX_CAPABILITY_DOMAINS } from '../limits.ts';
...
export const domainSummarySchema = {
  type: 'object',
...
export function formatDomains(result: AnySearchDomainsResponse): string {
  ...
  lines.push(
    ...result.domains.map(
      (domain) => `- ${domain.domain} (${domain.subDomainCount} sub-domains): ${domain.description}`,
    ),
  );
  return lines.join('\n');
}
...
export function formatSubDomains(result: AnySearchSubDomainsResponse): string {
  ...
  for (const domain of result.domains) {
    ...
    for (const subDomain of domain.subDomains) {
      ...
      const params = Object.entries(subDomain.params).sort(
        (left, right) =>
          (left[1].sortOrder ?? Number.MAX_SAFE_INTEGER) -
...
export function registerCapabilitiesTool(ctx: any, client: AnySearchClient): void {
  const toolDefinition = {
    ...
    async execute(args: any, exec: { signal?: AbortSignal }) {
      ...
      const result = await client.getSubDomains(
        parseCapabilityDomains(args.domains),
        exec?.signal,
      );
      return {
        ...
        domains: result.domains.map((domain) => ({
          ...
          subDomains: domain.subDomains.map((subDomain) => ({
            ...
            params: Object.fromEntries(
              Object.entries(subDomain.params).map(([param, info]) => [
                param,
                {
    ...
    presentCall: (args: any) => ({
      ...
      title:
        args?.domains === undefined
          ? 'AnySearch domains'
          : `AnySearch: ${(args.domains as string[]).join(', ')}`,
      kind: 'search',
      rawInput: (args?.domains as string[])?.join(', ') ?? 'all domains',
    }),
  };
