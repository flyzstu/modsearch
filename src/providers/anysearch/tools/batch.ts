...
import { type AnySearchClient, AnySearchClientError } from '../client.ts';
import { ANYSEARCH_TOOL_TIMEOUT_MS, MAX_BATCH_SEARCH_ITEMS } from '../limits.ts';
...
export { MAX_BATCH_SEARCH_ITEMS } from '../limits.ts';
...
export interface BatchToolItemArgs {
  query: string;
  maxResults?: number;
  tag?: string;
...
export const anySearchBatchInputItemSchema = {
  type: 'object',
...
export const anySearchBatchSuccessSchema = {
  type: 'object',
  additionalProperties: false,
  properties: {
...
export const anySearchBatchFailureSchema = {
  type: 'object',
  additionalProperties: false,
  properties: {
    index: { type: 'integer', required: true },
...
export const anySearchBatchOutputSchema = {
  type: 'object',
  additionalProperties: false,
  properties: {
    items: {
      type: 'array',
      required: true,
      items: { oneOf: [anySearchBatchSuccessSchema, anySearchBatchFailureSchema] },
    },
    summary: {
      type: 'object',
      required: true,
...
export function formatBatchSearchOutput(
  ...
  for (const item of output.items) {
    ...
    for (const result of item.results) {
      lines.push(
        `- [${result.title.length > 0 ? result.title : new URL(result.url).hostname}](${result.url})${
...
export async function executeBatchSearch(
  ...
  const items = await Promise.all(
    parsed.map(
      async (item, index): Promise<AnySearchBatchSuccess | AnySearchBatchFailure> => {
        try {
  ...
  return {
    ...
    renderedContentTruncated:
      parsed.reduce((total, item, index) => {
...
export function registerBatchSearchTool(
  ...
  const toolDefinition = {
    ...
    parameters: {
      ...
      properties: {
        items: {
          type: 'array',
          required: true,
          items: anySearchBatchInputItemSchema,
          description:
    ...
    output: {
      schema: anySearchBatchOutputSchema,
    ...
    presentCall: (args: any) => ({
      ...
      kind: 'search',
      rawInput: (args?.items ?? []).map((item: any) => item.query).join('\n'),
    }),
  };
