...
const API_KEY_PLACEHOLDERS = new Set(['ANYSEARCH_API_KEY', 'as_sk_your_key']);
...
export class AnySearchClient {
  ...
  async extract(
    request: AnySearchExtractRequest,
    signal?: AbortSignal,
  ...
  async getSubDomains(
    ...
    signal?: AbortSignal,
  ): Promise<AnySearchSubDomainsResponse> {
  ...
  private async request(
    ...
    try {
      ...
      throw new AnySearchClientError(`AnySearch ${operation} request failed: ${String(error)}`, {
        operation,
        cause: error,
      });
    }
...
function parseSearchData(envelope: EnvelopeData): AnySearchSearchResponse {
  const parsedResults = arrayField(envelope.data, 'results', 'data.results').map((value, index) =>
    parseSearchResult(value, index),
  );
  ...
  return {
    ...
    results,
    metadata: {
      totalResults: nonNegativeIntegerField(
        metadata,
        'total_results',
        'data.metadata.total_results',
      ),
      searchTimeMs: nonNegativeIntegerField(
        metadata,
        'search_time_ms',
...
function parseDomainsData(envelope: EnvelopeData): AnySearchDomainsResponse {
  const domains = arrayField(envelope.data, 'domains', 'data.domains').map((value, index) =>
    parseDomainSummary(value, index),
  );
  return {
    ...(envelope.requestId === undefined ? {} : { requestId: envelope.requestId }),
    domains,
...
function parseSubDomainsData(envelope: EnvelopeData): AnySearchSubDomainsResponse {
  const domains = arrayField(envelope.data, 'domains', 'data.domains').map((value, index) =>
    parseDomainCapability(value, index),
  );
  return {
    ...(envelope.requestId === undefined ? {} : { requestId: envelope.requestId }),
    domains,
...
function parseDomainCapability(value: unknown, index: number): AnySearchDomainCapability {
  ...
  return {
    domain: stringField(domain, 'domain', `${path}.domain`),
    description: stringField(domain, 'description', `${path}.description`),
    subDomains: arrayField(domain, 'sub_domains', `${path}.sub_domains`).map((item, subIndex) =>
      parseSubDomain(item, `${path}.sub_domains[${subIndex}]`),
    ),
  };
}
...
function parseSubDomain(value: unknown, path: string): AnySearchSubDomain {
  ...
  const paramsValue =
    subDomain.params === undefined ? {} : record(subDomain.params, `${path}.params`);
...
function optionalNumberRecordField(
  value: Record<string, unknown>,
  key: string,
  path: string,
): number | undefined {
...
function boundedUpstreamDetail(detail: string): string {
  const bounded =
    detail.length <= MAX_UPSTREAM_ERROR_CHARS
      ? detail
