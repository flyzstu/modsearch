...
Some harnesses forbid running scripts. Reason through the same order by hand and run the first line that works (the pinned version is 5.11.0):

1. A `modsearch` on `PATH` whose major version is 5 and is at least 5.11.0: `modsearch <args>`.
2. Otherwise, if `npx` exists: `npx --yes --package @liustack/modsearch@5.11.0 modsearch <args>`.
3. Otherwise, if `bunx` exists: `bunx --bun @liustack/modsearch@5.11.0 <args>`.
...
| :-- | :-- | :-- |
| search the web | `firecrawl`, `antigravity-cli`, `ollama`, `tavily`, `exa`, `brave`, `anysearch` | Firecrawl works keyless with no signup (1,000 free credits/month). agy is free with a browser sign-in. AnySearch, Ollama, Tavily, Exa, Brave, and a free Firecrawl key add personal quotas. |
| fetch a page | `firecrawl`, `antigravity-cli`, `ollama`, `anysearch`, `local` | Firecrawl, Ollama, and AnySearch run in the cloud, Firecrawl keyless by default (`firecrawl.keylessFetch false` opts out). `local` needs nothing and is the default floor. |
