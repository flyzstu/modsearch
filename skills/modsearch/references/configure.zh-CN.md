...
官方 Ollama Web Search & Fetch REST API。注册 Ollama 账号免费获取 key：https://ollama.com/settings/keys。

```bash
modsearch config set ollama.apiKey <key>
# 或环境变量：export OLLAMA_API_KEY=<key>
```

同时支持网页搜索（`/api/web_search`）和云端网页抓取（`/api/web_fetch`）。

### brave（搜索，免费额度）

每月 2,000 次请求，不绑卡。key 在 https://brave.com/search/api/ 领。

```bash
modsearch config set brave.apiKey <key>
# 或环境变量：export BRAVE_API_KEY=<key>
```

Brave Search 返回排好序的网页、附加摘要片段和站点 Profile。它的 summary 是机械拼的，证据在 `items` 里。
