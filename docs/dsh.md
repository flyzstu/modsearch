...
The dump should contain both `searchProvider: modsearch` and a plugin row named `@flyzstu/modsearch`.
...
```sh
npx -y @deepseek-ai/dsh plugin --profile web add @flyzstu/modsearch@5.11.1
```
...
```sh
npx -y @flyzstu/modsearch@5.11.1 doctor
```
...
- The route carries dsh's own browser fence, the one in front of its `/api`: same-origin requests to an authority this deployment serves (loopback, or a host declared in `client-connection.trustedHosts`, plus the LAN IP literals an all-interface bind derives), presented with the browser session every other surface uses. An untrusted authority or a cross-site request gets 403; a request with no verified browser session gets 401.
...
```sh
npx -y @deepseek-ai/dsh plugin --profile <name> add @flyzstu/modsearch@5.11.1
```

Remove it with:

```sh
npx -y @deepseek-ai/dsh plugin --profile <name> remove @flyzstu/modsearch
```
