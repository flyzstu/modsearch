...
输出中应同时出现 `searchProvider: modsearch` 和名为 `@flyzstu/modsearch` 的插件行。
...
dsh 网页端没有终端，所以插件会在「设置 → 插件」里挂一张**搜索引擎（ModSearch）**卡片（dsh 自带的「网页搜索」卡片是 DeepSeek 自己的搜索提供方，两者不是一回事）。它改的就是 CLI 改的那份 `~/.modsearch/config.json`，走插件注册的 `/modsearch/config` 路由。
...
- 路由使用 dsh 自己的浏览器围栏，也就是它 `/api` 前面那一道：同源请求且 authority 属于本部署所服务范围（loopback，或部署在 `client-connection.trustedHosts` 里声明的主机，全接口绑定还会自动信任其 LAN IP 字面量），并带上其他所有界面都在用的浏览器会话。authority 不受信或跨源请求返回 403；没有已验证浏览器会话则返回 401。
